...
**Date**：2026-07-19（UTC 时间戳版本 `20260719T043340Z`）
...
| **C2**（T1，核心） | 若 ACA 能在 O(1) 下逼近 RIT 标签并优于 correlational baseline，则「可负担的因果记功」这一主贡献成立 | held-out event-level `CCC(φ̂,φ̃)` 优于 baseline；memory-level `CCC(φ̄,Φ̃)`；calibration（reliability + `ECE_ACA_HELDOUT`）；split-half 可靠；成本落在上限内 | `fig:mechanism-recovery`(Fig 5a)；`tab:ablation`(Table 3) | **G-C2**（ROUND2：**合取**）：ACA-over-baseline 下界改善 ∧ event-level CCC ∧ memory-level CCC ∧ calibration ∧ cost 上限；`ρ≥0.6` 为 `SOURCE-DEFAULT` gate | C2 unsupported / revise scope | B3（主）, B5, B6 |
| **C3**（T1） | 若同一信号被极简规则消费即可压平 SR、降低 CTI，并可作 RL 奖励移植，则「一个信号、直接可消费」成立，且证明增益来自信号而非管道 | A2（swap causal→co-occurrence，增益应消失）、A3（no scope，CTI 应回升）、A4（no governance，SR 斜率应回升）；`CTI` 度量；reward substitution lift | `tab:reward-swap`(Table 2)；`tab:ablation`(Table 3)；`fig:mechanism-recovery`(Fig 5b) | **G-C3a**（Full-vs-A4 SR 斜率差 CI 排除 0）/ **G-C3b**（CTI 降低 CI 排除 0）/ **G-C3c**（reward 移植 lift CI 排除 0）；均 Holm 校正 | 丢弃对应子 claim / report null portability | B4（主）, B5 |
...
- **Main paper must prove（MUST-RUN 覆盖）**：
  ...
  - C3 消费者（B4 → Table 3 / Fig 5b）+ signal-portability（B5 → Table 2）。
...
- **Priority**：MUST-RUN（作为 C3 三消费者之一；亦是 simplicity 论证）。
...
- **Success criterion（G-H4）**：Pareto knee 按 `τ_knee` 规则存在；MERIT 位于前沿拐点。
- **Failure interpretation**：无 knee / MERIT 非 Pareto 最优 → 丢弃 Pareto claim。
- **Table / figure target**：`tab:efficiency`(Table 6)；`fig:streaming-efficiency`(Fig 4)。
- **Priority**：MUST-RUN（低开销 gate 支撑 C4）。
...
|---|---|---|---|---|---|
| **M0 Sanity** | 管道 + 指标正确 | B0 | 配对重跑逐位可复现 ∧ 三指标脚本过 toy → GO；否则修管道，不进 M1 | 待测（无虚构） | 指标口径错污染全下游 / 先锁 Fig2-Fig5 同坐标同统计 |
| **M1 Baseline** | 复现最强 baseline | B1 baseline rows | ReasoningBank/MemRL 复现落在 fidelity checklist（RM7）内 → GO | `[[TBD:MAIN_GPU_HOURS_PER_RUN]]`（待测） | baseline 不忠实 → C4 可比性 / 用 source-faithful vs standardized-wrapper 分列 |
...
| **M3 Main** | C4 主结果 + 效率 | B1 full, B7 | **Gate-C4**（合取）：max-baseline Holm CI 排除 0 ∧ 低开销 gate；否则 report parity / revise | `[[TBD:MAIN_GPU_HOURS_PER_RUN]]`（待测） | 增益不足撑不起 headline / 先改卡再改文（§13 迭代环） |
| **M4 Decision** | novelty/simplicity/frontier 消融 | B3, B4, B5, (B6) | **Gate-C2**（合取，`ρ≥0.6`+…）∧ **A2 gate**（换信号增益消失）∧ **G-C3a/b/c** | 待测 | ACA 噪声 → C2；A2 不消失 → 信号未定位 / §8.5 校准小节 |
| **M5 Polish** | 边界/机制/案例/附录 | B8, B9 | 每轴预测 sign 成立则报；redundancy 保持 `EVIDENCE GAP` | 待测 | 过度外推边界 / H5 只报通过的轴 |

**执行顺序总述**：`sanity(M0) → baseline(M1) → pilot(M2) → freeze(M2.5) → main(M3) → ablation/decision(M4) → analysis/polish(M5)`。
...
- [x] Simplicity 辩护：阈值规则 vs RL manager（B5 reward-swap）；full Shapley/RL 升级已 CUT。
- [x] Frontier 贡献辩护或明确不主张：B6（usage-feature A5、A-judge）作 NICE-TO-HAVE 附录。
- [x] Nice-to-have（B6/B8/B9）与 must-run（B0–B5,B7）分离。
