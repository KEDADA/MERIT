...
> 未运行任何实验；本表仅登记待执行 runs。`Priority`：MUST / NICE。UTC 版本 `20260719T043340Z`。
...
| R003 | M0 | sanity：judge 一致性口径 | GPT-5.1 judge vs 人工 200 例 | 抽检子集 | JUDGE_HUMAN_AGREEMENT_KAPPA | MUST | TODO | κ 口径待定（USER_APPROVAL） |
...
| R012 | M1 | 复现其余 baseline 行 | NoMem/FullHist/Mem0/A-MEM/AWM/SkeMex/MemR1/Mem-α/Reflexion/RBANK+MaTTS | 主基准 4 类 | MAIN_{NOMEM,FULLHIST,MEM0,AMEM,AWM,SKEMEX,MEMR1,MEMALPHA,REFLEXION,RBANKMATTS}_* | MUST | TODO | 14 行主表除 MERIT/RITFULL |
...
| R055 | M4 | reward-swap 移植 | MemR1/Mem-α × {orig, φ̂} | GRPO 不变 | PLUGIN_MEMR1_ORIG_AVG, PLUGIN_MEMR1_PHI_AVG, PLUGIN_MEMALPHA_ORIG_AVG, PLUGIN_MEMALPHA_PHI_AVG, REWARD_SWAP_MEMORY_R1_GAIN, REWARD_SWAP_MEMALPHA_GAIN | MUST | TODO | **G-C3c**；Table 2；RL 定位互补 |
...
| R060 | M3/M4 | 效率 Pareto | baseline/MERIT/A1/RIT-Full | budget sweep p∈{1,2,5,10}% | EFF_BASELINE_AVG, EFF_MERIT_AVG, EFF_A1_AVG, EFF_RITFULL_AVG, MERIT_RELATIVE_TOKEN_COST, A1_RELATIVE_TOKEN_COST, RITFULL_RELATIVE_TOKEN_COST, FULL_RIT_COST_MULTIPLIER | MUST | TODO | **G-H4** knee（τ_knee 待定）；Table 6 |
...
**说明**
- Run ID 段位：R00x=sanity(M0)，R01x=baseline(M1)，R02x/R03x=pilot/freeze(M2/M2.5)，R04x=main(M3)，R05x=ablation/decision(M4)，R06x=efficiency，R07x/R08x=boundary/case(M5)。
